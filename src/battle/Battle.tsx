...
import { generateBattleChain } from "../data/generateBattle";
import "./battle.css";
...
export default function Battle({ onRestart }: { onRestart?: () => void }) {
  const chain = useMemo(() => generateBattleChain(), []);
  ...
  function handlePick(code: string) {
    ...
    const higher = champion.population >= challenger.population ? champion : challenger;
    ...
    return (
      <div className="bat-game">
        <header className="bat-header">
          ...
          <div className="bat-category-label">Population</div>
        </header>
  ...
  return (
    <div className="bat-game">
      ...
      <div className="bat-question">Which country has more people?</div>
      ...
      <div className="bat-arena">
        ...
        >
          ...
          <span className="bat-card-value">{formatPopulation(champion.population)}</span>
        </button>
        ...
        >
          ...
          {roundPhase === "feedback" ? (
            <span className="bat-card-value">{formatPopulation(challenger.population)}</span>
          ) : (
